...
namespace Application.Profiles.Queries
{
    public class GetProfilePhotos
    {
        ...
        public class Handler(AppDbContext context) : IRequestHandler<Query, Result<List<Photo>>>
        {
            ...
            {
                var photos = await context.Users.Where(x => x.Id == request.UserId).SelectMany(x => x.Photo).ToListAsync(cancellationToken);
                return photos is null ?
                    Result<List<Photo>>.Failure("Photos not found", 404) :
                    Result<List<Photo>>.Success(photos);
            }
        }
    }
}
